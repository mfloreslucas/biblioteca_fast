...
export default function Usuarios() {
  ...
  useEffect(() => {
    setLoadingRaw(true);
    fetch('/api/usuarios/raw').then(r => r.json()).then(data => {
      setRaw(data);
      setLoadingRaw(false);
    });
    setLoadingOpt(true);
    fetch('/api/usuarios/optimized').then(r => r.json()).then(data => {
      setOpt(data);
      setLoadingOpt(false);
    });
  }, []);
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6 text-[#183153]">Usuarios</h2>
      <div className="mb-8">
