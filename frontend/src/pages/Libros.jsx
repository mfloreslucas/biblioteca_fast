...
export default function Libros() {
  ...
  useEffect(() => {
    setLoadingRaw(true);
    fetch('/api/libros/raw').then(r => r.json()).then(data => {
      setRaw(data);
      setLoadingRaw(false);
    });
    setLoadingOpt(true);
    fetch('/api/libros/optimized').then(r => r.json()).then(data => {
      setOpt(data);
      setLoadingOpt(false);
    });
  }, []);
  ...
  return (
    <div className="p-6">
      <h2 className="text-2xl font-bold mb-6 text-[#183153]">Libros</h2>
      <div className="mb-8">
